...
class _MathMazeScreenState extends ConsumerState<MathMazeScreen> {
        ...
        if (r == _gridSize - 1 && c == _gridSize - 1) {
          if (_runningTotal == _currentPuzzle.target) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Reached end but total does not match target!'), duration: Duration(seconds: 1)),
            );
            // Reset to start
            _userPath = [const Point(0, 0)];
  ...
  Widget build(BuildContext context) {
    ...
    return GameScaffold(
      title: 'MATH MAZE',
      subtitle: 'Path from top-left to bottom-right evaluating to target',
      actions: [
        IconButton(
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Column(
                  children: [
                    const Text('TARGET', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
                ...
                Column(
                  children: [
                    const Text('CURRENT TOTAL', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
                    const SizedBox(height: 4),
                    Text(
                      '$_runningTotal',
                      style: Theme.of(context).textTheme.headlineMedium?.copyWith(

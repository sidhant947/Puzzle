...
import 'common_denominator_provider.dart';

class CommonDenominatorScreen extends ConsumerStatefulWidget {
  const CommonDenominatorScreen({super.key});

  @override
  ConsumerState<CommonDenominatorScreen> createState() => _CommonDenominatorScreenState();
}

class _CommonDenominatorScreenState extends ConsumerState<CommonDenominatorScreen> {
  @override
  void initState() {
    super.initState();
    Future.microtask(
  ...
  void _showGameOverDialog(int score) {
    ...
    final won = score >= 5;
    if (won) {
      ref.read(gameStreakNotifierProvider.notifier).completeGame('common_denominator');
    }
    showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(commonDenominatorNotifierProvider);
    final notifier = ref.read(commonDenominatorNotifierProvider.notifier);

    ref.listen(commonDenominatorNotifierProvider, (previous, next) {
    ...
    return GameScaffold(
      title: l10n.commonDenominatorTitle.toUpperCase(),
      subtitle: l10n.commonDenominatorSubtitle,
      body: LayoutBuilder(

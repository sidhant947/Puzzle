...
import 'percentage_peak_provider.dart';

class PercentagePeakScreen extends ConsumerStatefulWidget {
  const PercentagePeakScreen({super.key});

  @override
  ConsumerState<PercentagePeakScreen> createState() => _PercentagePeakScreenState();
}

class _PercentagePeakScreenState extends ConsumerState<PercentagePeakScreen> {
  @override
  void initState() {
    super.initState();
    Future.microtask(
  ...
  void _showGameOverDialog(int score) {
    ...
    final won = score >= 5;
    if (won) {
      ref.read(gameStreakNotifierProvider.notifier).completeGame('percentage_peak');
    }
    showDialog(
      context: context,
  ...
  Widget build(BuildContext context) {
    ...
    final state = ref.watch(percentagePeakNotifierProvider);
    final notifier = ref.read(percentagePeakNotifierProvider.notifier);

    ref.listen(percentagePeakNotifierProvider, (previous, next) {
    ...
    return GameScaffold(
      title: l10n.percentagePeakTitle.toUpperCase(),
      subtitle: l10n.percentagePeakSubtitle,
      body: LayoutBuilder(
  ...
  Widget _buildNumberPad(PercentagePeakNotifier notifier, BoxConstraints constraints) {
    return Column(
      children: [
